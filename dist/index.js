...
const stocks = [
    ...
    ["ZAL.DE", "Zalando SE"],
].map(([symbol, name]) => new stock_1.Stock(symbol, name));
const wallet = new wallet_1.Wallet(50000, stocks); // 50k
async function live() {
    await browser_1.start(config.website.username, config.website.password, wallet);
    data_1.liveData(stocks);
    new cron_1.CronJob("* * * * *", () => transaction_1.transaction(wallet), null, true);
}
async function history() {
    await data_1.historyData(wallet);
    await wallet.sell(...stocks);
    console.log(wallet.money);
}
async function chat() {
    if (config.matrix !== undefined) {
        await chat_1.chatBot(config.matrix.userId, config.matrix.accessToken, config.matrix.server, wallet);
    }
}
live();
// history();
